...
│   ├── heimdall-descriptor.ts    # Hugin-owned GET /heimdall.json typed-panel descriptor (#116/#135)
...
│   ├── version-drift.ts          # Version-drift self-check (#123), PURE: build/compare on-disk @anthropic-ai/claude-agent-sdk + cli.js snapshots. Detects deps bumped under a live worker (the 2026-06-17 incident class). IO (fs reads, startup baseline, alerting) lives in index.ts, same split as auth-alarm.ts/probeClaudeUsage
...
│   │   └── tools.ts              # Delegation, learning-loop, and friction MCP tools
...
| `HUGIN_VERSION_DRIFT_CHECK` | `on` | Version-drift self-check (issue #123). `on` snapshots the on-disk `@anthropic-ai/claude-agent-sdk` version + vendored `cli.js` identity (size/mtime) once at worker startup (`src/version-drift.ts`), then re-reads and compares before every `claude`-runtime SDK task. A confirmed mismatch — deps bumped on disk under a live worker, the 2026-06-17 incident class — refuses the task fast with a distinctly-classified `DEPS_DRIFT` failure (tag `failure:deps-drift`, `- **Failure kind:** DEPS_DRIFT`) instead of an opaque `exit 1`, and pushes one edge-triggered alert through the same Ratatoskr Alert Bus path as `HUGIN_AUTH_ALARM` (reuses its send-target env vars below). The check itself is always fail-open: any error reading on-disk state (missing baseline, mid-write file, permissions) is logged and treated as no-drift — only a genuine, confirmed mismatch ever blocks a task. Set to `off` to disable. |
