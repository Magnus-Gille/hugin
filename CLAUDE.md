...
| `HUGIN_AUTH_ALARM` | `on` | Proactive Pi Claude credential-expiry alarm (issue #131). `on` runs a periodic probe (the same OAuth-usage check the pre-flight uses) and, on an **edge transition** to `unauthorized` — or when the token is nearing expiry — pushes an alert to the user via Ratatoskr's Alert Bus (`POST /api/send` → Telegram + Heimdall echo). Edge-triggered: fires once on `ok→unauthorized`, once on recovery, once per token on impending expiry — never every tick. State persists to Munin (`tasks/_auth_alarm`) so a restart doesn't re-fire. Inert (probes + logs, no push) unless the Ratatoskr send target below is configured. `off` disables. See `src/auth-alarm.ts`. |
| `HUGIN_AUTH_ALARM_INTERVAL_MS` | `3600000` (1h) | Cadence of the auth-alarm probe. |
| `HUGIN_AUTH_ALARM_EXPIRY_WARN_MS` | `43200000` (12h) | Warn this far ahead of the credential's `expiresAt`. |
