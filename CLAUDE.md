...
| `HOMESERVER_GATEWAY_URL` | — | Root URL of the M5 local-inference gateway (e.g. `http://100.76.72.59:8080`, no `/v1`). Enables the orchestrator's `homeserver` provider (base URL resolved at request time, `/v1` appended) and the standalone `homeserver-executor.ts`. The host is auto-added to the egress allowlist. |
